...
export namespace API {
  ...
  export type VoteOnCommentAPIResponse = APIResponse<DTOs.CommentVoteDTO, VoteErrors>;
...
export const createVotesAPI = (apiUrl: string) => {
  return {
    voteOnComment: async (input: Inputs.VoteOnCommentInput, authToken: string): Promise<API.VoteOnCommentAPIResponse> => {
      try {
        const successResponse = await axios.post(`${apiUrl}/votes/comment/${input.commentId}/new`, input, {
          headers: { Authorization: `Bearer ${authToken}` }
        });
        return successResponse.data as API.VoteOnCommentAPIResponse;
      } catch (_err: unknown) {
        if (axios.isAxiosError(_err) && _err.response) {
          return _err.response.data as API.VoteOnCommentAPIResponse;
        }
        return {
          data: undefined,
          error: "Unknown error",
          success: false
        } as API.VoteOnCommentAPIResponse;
      }
    },
