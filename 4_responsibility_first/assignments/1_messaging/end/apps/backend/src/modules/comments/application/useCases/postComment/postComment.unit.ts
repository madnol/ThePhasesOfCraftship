...
describe('postComment', () => {
  ...
  let commentsRepo = new ProductionCommentsRepository(database, outbox);
  ...
  const useCase = new PostComment(commentsRepo, postsRepo, membersRepo);
  ...
  describe('permissions & identity', () => {
    ...
    test('as a level 1 member, I should be able to post a comment', async () => {
      ...
      expect(commentPostedEvent.data.commentId).toBe(response.getValue()!.id);
      expect(commentPostedEvent.data.postId).toBe(existingPost.id);
      expect(commentPostedEvent.data.memberId).toBe(level1Member.id);
    });
  });
